...
export default class HbookerService {
    ...
    async getBookList({ bookshelf, noCache }) {
        ...
        let rawBooks;
        if (!noCache) {
            const cacheData = await db.books.where("shelfId").equals(bookshelf.shelfId).first();
            if (cacheData)
                rawBooks = cacheData.data;
        }
    ...
    async getVolumeList({ book, noCache }) {
        ...
        let volumes;
        if (!noCache) {
            const cacheData = await db.volumes.where("bookId").equals(book.bookId).first();
            if (cacheData)
    ...
    async getChapterDetail({ book, chapter, noCache }) {
        ...
        let chapterDetail;
        if (!noCache) {
            const cacheData = await db.chapterDetails.where("chapterId").equals(chapter.chapterId).first();
            if (cacheData)
                chapterDetail = cacheData.data;
        }
        if (!chapterDetail) {
